...
export type AuditTrailProps = {
  ...
  storageMode: RuntimeStorageMode | null;
  isTampering: boolean;
  isResetting: boolean;
  isBusy: boolean;
...
export function AuditTrail({
  ...
  const historicalOperations = operations.slice(0, -1).reverse();
  const selectedHistoricalOperation = historicalOperations.find(
    (operation) => operation.id === Number(targetOperationId)
  );
  const effectiveTargetOperationId = selectedHistoricalOperation ? targetOperationId : "";
            ...
            <div className="row g-3 align-items-end mt-1">
              ...
              <div className="col-md-6">
                ...
                <select
                  className="form-select"
                  id="tamper-target"
                  value={effectiveTargetOperationId}
...
function TamperReceipt({
  ...
  return (
    <div className="tamper-receipt border rounded-3 p-3 mt-4">
      ...
      {copyStatus && <div className="small text-secondary mt-3" role="status">{copyStatus}</div>}
    </div>
  );
}
